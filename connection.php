...
function update_IF($user, $pass, $name, $email, $phone, $year,$IdAccount,$role){
  $conn = open_database();
  $Quyen = $role;
  $hash = password_hash($pass, PASSWORD_DEFAULT);
  $sql = "UPDATE account SET HoTen=?,UserName=?,Email=?,Sdt=?,Pass=?,NamSinh=?,Quyen=? WHERE IdAccount = $IdAccount";
  $stm = $conn->prepare($sql);
  $stm->bind_param('sssssss', $name, $user, $email, $phone, $hash, $year,$Quyen);
  if (!$stm->execute()) {
    return array('code' => 2, 'error' => 'can not execute command');
  }
...
function create_assignment($IdLop, $title, $content, $file, $datestart, $deadline)
{
  $sql = 'INSERT INTO assignment(IdLop,TieuDe,NoiDung,File,NgayBatDau,Deadline) values (?,?,?,?,?,?)';

  $conn = open_database();
  $stm = $conn->prepare($sql);
  echo $conn->error;
  $stm->bind_param('isssss', $IdLop, $title, $content, $file, $datestart, $deadline);
  if (!$stm->execute()) {
    return array('code' => 2, 'error' => 'can not execute command');
  }
  return array('code' => 0, 'error' => 'Create account successful');
